...
UPDATE DEPARTMENT2 
SET DEPT_TITLE = '기술 연구팀';

SELECT * FROM DEPARTMENT2 d ;
-- 조건을 설정하지 않으면 지정된 테이블의 모든 행이 수행된다 !!!!!!!!!!!
-- WHERE절 작성을 잘하자 
ROLLBACK;
...
-- 유재식의 급여 조회
SELECT SALARY FROM EMPLOYEE2 e WHERE EMP_NAME = '유재식'; --340만

-- 유재식의 보너스 조회
SELECT BONUS FROM EMPLOYEE2 e WHERE EMP_NAME = '유재식';--0.2
...
SELECT SALARY ,BONUS  FROM EMPLOYEE2 e WHERE EMP_NAME = '방명수'; --138
ROLLBACK;
...
-- EMPLOYEE 테이블에서 아시아지역에 근무하는 직원의 보너스를 0.3으로 변경
