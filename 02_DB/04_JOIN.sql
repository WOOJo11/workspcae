...
FROM DEPARTMENT d JOIN LOCATION ON (LOCATION_ID=LOCAL_CODE);

-- 오라클 방식
SELECT DEPT_TITLE,LOCAL_NAME
FROM DEPARTMENT d ,LOCATION 
...
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE 
FROM EMPLOYEE e LEFT OUTER JOIN DEPARTMENT d ON(DEPT_CODE = DEPT_ID);
-- 기존 INNER JOIN 결과에 
...
SELECT EMP_NAME,DEPT_CODE, DEPT_ID, DEPT_TITLE 
FROM EMPLOYEE e FULL OUTER JOIN DEPARTMENT d ON(DEPT_CODE = DEPT_ID);
...
-- * JOIN 순서 중요!!!* 
--> 앞 선 두 테이블 (EMPLOYEE, DEPARTMENT)의 조인 후 
...
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE SUBSTR(EMP_NO,1,1) = 7 AND EMP_NAME LIKE '전%'
...
SELECT EMP_NAME AS 사원명 ,DEPT_TITLE AS 부서명, LOCAL_NAME AS 지역명,
NATIONAL_CODE AS 국가명 
FROM EMPLOYEE  
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
