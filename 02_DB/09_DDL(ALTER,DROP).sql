...
CHK_DEPT_COPY CHECK(LOCATION_ID IN ('L1','L2','L3','L4','L5'));
...
-- > NOT NULL 은 제약조건 ADD는 안되는데 DROP 은가능함

-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY MODIFY CONSTRAINT SYS_C007520 NULL;
...
-- DEPT_COPY 의 LNAME 컬럼의 모든 값을 기본 값으로 수정
UPDATE DEPT_COPY 
SET LNAME = DEFAULT;
...
-- TCL : 트랜잭션 제어언어 (DML수행 내용을 COMMIT ROLLBACK SAVEPOINT)
