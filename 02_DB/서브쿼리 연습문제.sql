-- 1번 전지연 사원이 속해있는 부서원들을 조회하라 전지연 제외
-- 사번 사원명 전화번호 고용일 부서명
...
SELECT EMP_ID,EMP_NAME,JOB_NAME,FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE('19'||
SUBSTR(EMP_NO,1,6),'YYYYMMDD'))/12) AS 나이 , 
TO_CHAR((SALARY+(SALARY*NVL(BONUS,0)))*12,'L999,999,999') AS "보너스 포함 연봉"
FROM EMPLOYEE MAIN 
JOIN JOB J ON (J.JOB_CODE = MAIN.JOB_CODE)
WHERE EMP_NO  = (SELECT MAX(EMP_NO)   FROM EMPLOYEE SUB 
