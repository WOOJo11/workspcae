...
VALUES('MEM04','12341234','이지연','001101-4564567');

--  데이터  삽입 확인
SELECT * FROM MEMBER;
COMMIT; -- TCL로서 
		-- 트랜잭션에 저장된 DML 구문을 DB에 반영 저장공간 !

ROLLBACK; -- 마지막 COMMIT상태로 돌아감 (트랜잭션 저장된 DML 삭제)

-----------------------------------------------------------
-- 주민등록번호, 전화번화와 같은 숫자로만 이루어진 데이터를 
...
INSERT INTO MEMBER2
VALUES( '고길동', 9612022345678,01012341234);
INSERT INTO MEMBER2
VALUES( '김길동', 0001013345678,01012341534);

SELECT * FROM MEMBER2;
-- NUMBER 타입의 컬럼은 값 제일 앞 0을 유지하지 않는 문제 발생
-- 문제해결 방법 :CHAR 또는 VARCHAR2 타입으로 컬럼타입을 지정
...
CREATE TABLE USER_USED_UK(
   ...
   -- USER_ID VARCHAR2(20) CONSTRAINT USER_ID_UK UNIQUE ,
    ...
    EMAIL VARCHAR2(50),
 	-- 테이블 레벨 제약조건 설정
    --UNIQUE(USER_ID) -- 제약 조건(컬럼명) (이름 미지정)
		CONSTRAINT USER_ID_UK UNIQUE(USER_ID) -- 이름 있음
    );

-- 테이블 삭제할 때 DROP 사용 DELETE 아님
...
CREATE TABLE USER_USED_PK(
    --USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼레벨(제약조건명 X)
    						--컬럼 레벨 제약 조건 O 오류가 떴을때 확인가능 
    USER_NO NUMBER,
    ...
    EMAIL VARCHAR2(50),
 	--테이블 레벨을 가장 마지막에 
    CONSTRAINT USER_NO_PK PRIMARY KEY(USER_NO) -- 제약조건 명 X 
...
WHERE GRADE_CODE ='20';
 -- GRADE_CODE = 20번 행은 자식 테이블에서 참조하고 잇지 않으므로 
-- 삭제가 제한되지 않는다 

-- 참조를 하고 있는 GRADE_CODE 10을 삭제
 DELETE FROM USER_GRADE 
WHERE GRADE_CODE ='10';
-- 자식레코드가 발견되었다 ? USER_USER_FK 에서 사용하고있음
...
VALUES (1, 'user1', '1234', '921120-1234567', '남', '010-1111-9999', '서울시 성북구 동선동', 'N');

INSERT INTO USER_TEST
VALUES (2, 'user2', '1234', '961202-20001111', '여', '010-2222-9999', '서울시 성북구 안암동', 'N');

INSERT INTO USER_TEST
VALUES (3, 'user3', '1234', '961225-2220001', '여', '010-3333-9999', '서울시 성북구 삼선동', 'Y');

INSERT INTO USER_TEST
VALUES (4, 'user4', '1234', '990424-1234567', '남', '010-4444-9999', '서울시 성북구 종암동', 'N');

INSERT INTO USER_TEST
VALUES (5, 'user5', '1234', '980325-2234567', '여', '010-5555-9999', '서울시 성북구 길음동', 'N');
...
--AS 서브쿼리; 서브쿼리결과 result set 의 형태로 테이블 생성 
