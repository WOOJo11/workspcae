...
public class EmployeeView {
	...
	public void displayMenu() {
		...
		try {
			...
			System.out.println("1. 전체 사원 조회"); // 사번, 이름, 부서명, 직급명, 전화번호
													 // 직급코드 오름차순			
			System.out.println("2. 사번이 일치하는 사원 조회 "); // 한명만 조회
			// 사번, 이름, 부서명, 직급명, 전화번호 1행 조회
			
			System.out.println("3. 이름 글자가 포함된 사원 검색");
			// 사번, 이름, 부서명, 직급명, 전화번호 N행 조회
			// 사번 오름 차순
			
			System.out.println("4. 급여 범위로 조회");
			// 사번 이름 직급명 급여를 조회 (최대 최소 값 지정) 
			// 급여 내림차순 으로 
			...
			input = -1; // while 문이 종료되지 않게하기 위한 값 대입
	...
	private void selectName() {
		...
		try {
			...
			empList = service.selectName(input);
			...
			System.out.println("입력된 글자가 이름에 포함된 사원 조회 중 예외 발생"); 
	...
	private void selectSalary() {
		...
		System.out.print("시작 범위를 입력하세요 : ");
		int input1 = sc.nextInt();
		
		System.out.print("원하는 범위를 입력하세요 : ");
		...
		try {
			...
			System.out.println("급여 범위로 조회 중 오류발생");
			e.printStackTrace();
		}
	}
    ...
    Employee emp = new Employee(empName, empNo, email, phone, salary, deptCode, jobCode,
    		salLevel, bonus, managerId);
  
    
    
    
    // 사원 정보를 DB에 삽입하는 서비르를 호출 후 결과 반환 받기
    try {
