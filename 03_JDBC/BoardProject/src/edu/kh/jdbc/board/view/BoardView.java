	   ...
	   public void boardMenu() {
		      ...
		      do {
		         try {
		            ...
		            // 제목 , 내용 (StringBuffer) 입력
		            // -> 게시글 삽입 서비스 (재목,내용 로그인 회원 번호) 호출
...
//		            case 4:  searchBoard(); break; // 게시글 검색
	...
	private void selectBoard() {
		...
		try {
			...
			// 입력받은 번호 로그인 한 회원 번호(조회수 증가 체크에 필요)
	...
	private void updateBoard(int boardNo) {
		...
		try {
			int result = boardService.updateBoard(title,sb.toString(),boardNo);
	...
	private void insertBoard() {
		...
		try {
			int result = boardService.insertBoard(title,sb.toString(),Session.loginMember.getMemberNo());
			if(result > 0) {
