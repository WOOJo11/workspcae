...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// JDBC : Java에서 DB에 연결 할 수 있게 해주는 
		// 		  코드(class , interface)를 제공해 주는 것
		//        --> Java Programming API
		// 		  --> java.sql 패키지에서 제공
		
		
		/* ** JBDC를 이용한 애플리케이션 제작 시 필요한 것 ** 
		 *  
		 *  1. Java의 JBDC 관련 인터페이스
		 *  2. DBMS의 종류 (Oracle)
		 *  3. DBMS 회사에서 제공하는 
		 *     Java 애플리케이션과 DB연결을 위한 라이브러리
		 *     (ojdbc11.jar -> OracleDrive.class)
		 */
		...
		// DB 연결 정보를 담는 객체 
		// --> null에는 Connection을 상속 받아 구현한 자식객체가 대입될 예정
		...
		// 조회 결과의 집합을 나타냄
		// -> 테이블 모양으로 반환
		// -> Java는 한번에 다 볼 수 없어 ResultSet 객체를 이용해 1행씩 접근
		// -> 더이상 접근할 행이 없을 경우 false 반환
		
		
		try { 
			// 2 단계 : 1단계에서 선언한 참조 변수에 알맞은 객체 대입
			...
			// 5. Statement 객체를 통해 SQL을 수행하고 
			//    결과(ResultSet)를 반환 받아아서 저장 (rs 변수 생성)
			...
			while(rs.next()) {
				...
				// rs.next() ? 이건 뭘까?
				// - ResultSet 객체는 조회 결과를 한번에 다루는 것이 아닌 
				//   커서라는 것을 이용해 1행씩만 접근
				// - 커서를 다음 행으로 이동 후 
