...
public class SwitchEx {
	 ...
	 * 
	 * [작성법]
	 * Switch(식){// 식 수행 결과는 true/false가 아닌 값(정수, 문자열 많이씀)
	 * 
	 * case 결과값1 : 수행코드1; breack;
	 * case 결과값2 : 수행코드2; breack;
	 * case 결과값3 : 수행코드3; breack;
...
//		}
		...
		//2) switch 버전 -> 스위치는 무조건 if로 바꿀수 있으나 
		//  if에서 스위치는 특수한 상황이 필요 같은 경우
	...
	public void ex2() {
		...
		// * Java에서 랜덤한 수(난수)를 발생시키는 방법
		//Math.random() : 난수 발생
		// 1) 난수 발생 범위 :0.0 <= random < 1.0
		// 2) 발생한 난수의 자료형 : double
		// + double -> 정수 강제형변환 : 소수점 버림
	...
	public void ex4() {
		...
		int num2 = sc.nextInt();
		//변수값?이라고해야하나 이것을 정할때 간소화 하면 아주 좋음
		...
		case "*" : System.out.printf( "%d %s %d =%d", num1, op, num2,  num1 * num2 ); break;
		case "/" : 
			if(num2 == 0) {System.out.println("0으로는 나눌 수 없습니다");}
			else
