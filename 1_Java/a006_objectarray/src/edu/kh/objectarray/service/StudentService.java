...
public class StudentService {
		...
		// Student 객체를 참조변수 5개짜리 배열을 생성해서 studentArr 참조
		private Student[] studentArr = new Student[5];
		
		//StudnetService 기본 생성자
		
		public StudentService() {
			
			//studentArr 배열의 각 요소는 Student 참조 변수
			studentArr[0] = new Student(3,5,7,"홍길동");
		...
		public boolean addStudent(int grade , int classRoom , int number, String name) {
			...
			// 해당 인덱스에 매개변수를 이용해서 생성된 Student 객체를 주소를 대입
			// -> true
		...
		public Student selectIndex(int index) {
			// index 값이 0~4사이가 아니면 
			// 배열 범위를 초과 했다는 ArrayIndexOutOfBoundException발생
		...
		public Student[] selectName(String inputName) {
			...
			//resultArr에 
				...
				// -> studnetArr[i] 값 대입 후 index 값 증가 (후위연산)
				
			}
			
			}
			// 이름이 일치 하는 학생이 없어서 index가 증가하지 않았다면
			if(index == 0) {
				return null; // null을 반환 시킨다
			}
				return resultArr;
