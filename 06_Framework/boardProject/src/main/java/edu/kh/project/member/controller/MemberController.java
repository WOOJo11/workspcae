...
@SessionAttributes({ "loginMember" }) // Model의 이름(key)를 적으면 session으로 처리 추가
...
public class MemberController {
	...
	public String login(HttpServletRequest req) {
		...
		// -> Spring Framework 제공하는 Argument Resolver가 해결해주기때문
	...
	public String login(/* @RequestParam("inputEmail") */ String inputEmail,
		...
		// @RequestParam("value="name", required="false", defaultVaule="1")
		// [속성]
	...
	public String login(/* @ModelAttribute */ Member inputMember) {
		...
		// 파라미터 전달 방법 3 : @ModelAttribute를 이용하는 방법
		...
		// - DTO에 기본생성자가 반드시 필요하다
		// - DTO에 setter가 반드시 필요하다
	...
	public String login(Member inputMember, Model model, @RequestHeader(value = "referer") String referer,
		...
		// -> 데이터를 K : V 형식으로 담아 전달
		// -> 기본적으로 request scope
		// -> @SessionAttributes 어노테이션과 함께 사용 시 sessionScope로 변경

		// @RequestParam(value="saveId",required=false) String saveId)
		// -> name 속성 값이 "saveId"인 파라미터를 전달 받아 저장
		// -> required=false : 필수 아님(null) 허용

		// 주의!!!! required 미작성시 기본 값 ture
		// -> 파라미터가 전달되지 않는 경우 주의!
		...
		// login 결과에 따라 redirect 경로를 다르게 지정
		...
		if (loginMember != null) { // 로그인 성공
			...
			/*
			 * Session - 서버가 클라이언ㅌ의 정보를 저장하고 있음 (쿠키와의 차이점)
			 * 
			 */
			...
			/*
			 * redirect(재요청) 시 기존 요청(request)이 사라지고 새로운 요청을 만들게 되어
			 * 
			 * redirect된 페이지에서는 이전 요청이 유지되지 않는다!!!! -> 유지하고 싶으면 어쩔 수 없이 Session을 이용
			 * 
			 * [Spring] 이런 상황을 해결하기 위한 객체 RedirectAttributes를 제공
			 * 
			 * RedirectAttributes - redirect시 데이터를 request scope로 전달할 수 있게하는 객체
			 * 
			 * 응답 전 : request scope 응답 중 : session scope 잠시 이동 응답 후 : request scope
			 * 
			 * 
			 */
	...
	public String signUp() {
		...
		// -> ViewResolver가 prefix, suffix를 리턴 값 앞뒤 붙여줌 착한친구
...
//		 inputMember.setMemberAddress(addr);
		...
		// 회원 가입 서비스 호출
		// (DB에 DML 수행 시 성공 행의 개수(반환))
		
		int result = service.signUp(inputMember);

		
		// 가입 성공 여부에 따라 이동하는 주소 설정
		String path = "redirect:";
		String message = null;
	...
	/*
	 * 스프링 예외처리 방법 (세 종류 , 우선 순위, 중복 사용)
	 * 
	 * 1순위 : 메서드 단위로 처리 -> try-catch / throws
	 * 
	 * 2순위 : 클래스 단위로 처리 -> @ExceptionHandler
	 * 
	 * 3순위 : 프로그램 단위(전역) 처리 -> @Controller 요청 시 응답해주는데 -> @ControllerAdvice >조언
	 */
	...
	public String exceptionHandler(Exception e, Model model) {
		...
		// forward 진행
		// -> View Resolver의 prefix, suffix를 붙여 JSP경로 만듦
		return "common/error";
