...
memberEmail.addEventListener("input",()=>{
    ...
    if(regEx.test(memberEmail.value)){ // 유효한 경우
        ...
        .then(Response =>Response.text()) // 응답객체 -> 파싱(데이터 형태변환)

        // 파싱한 데이터를 이용해 수행할 코드 작성
        .then(count => {
            // 카운트 중복되면 1 , 중복 아니면 0    
            if(count == 0){
                emailMessage.innerText="사용 가능한 이메일입니다";
                emailMessage.classList.add("confirm"); // 클래스추가 ,스타일적용
                emailMessage.classList.remove("error"); //스타일 제거
                checkObj.memberEmail=true;
            
            }else{
                emailMessage.innerText="이미 사용중인 이메일입니다";
                emailMessage.classList.add("error"); //스타일 제거
                emailMessage.classList.remove("confirm"); // 클래스추가 ,스타일적용
...
memberPw.addEventListener("input",()=>{
    ...
    if(memberPw.value.trim().length==0){
        ...
        pwMessage.classList.remove("confirm","error"); // 검정글씨로 바꿈

        checkObj.memberPw = false; // 빈칸이기에 유효 x
...
memberPwConfirm.addEventListener("input",()=>{
    ...
    // 비밀번호 == 비밀번호 확인 
    if(checkObj.memberPw){// 비밀번호가 유효하게 작성된경우
...
document.getElementById("signUpFrm").addEventListener("submit",e=>{
    ...
    for(let key in checkObj){
        ...
        if(!checkObj[key]){ // 각 key에 대한 value를 얻어
                            // false인경우 ==유효하지 않다
            ...
            // 유효하지 않은 input 태그로 focus이동
            // - key를 input의 id와 똑같이 설정
            document.getElementById(key).focus();
            e.preventDefault(); // form 태그 기본이벤트 제거
            return; // 함수 종료
        }
    }
